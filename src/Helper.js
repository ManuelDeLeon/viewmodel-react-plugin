export default class Helper {
  ...
  removeQuotes(str) {
    if (!str || !this.isString(str) || !(str[0] === '"' || str[0] === "'"))
      return str;
  ...
  reactStyle(str) {
    ...
    for (let block of str.split("-")) {
      ...
      if (
        !retVal &&
        (block.toLowerCase() === "ms" || str.substr(0, 1) !== "-")
      ) {
  ...
  isViewModel() {
    ...
    return (
      ...
      p.node.arguments[0].type === "ObjectExpression" && // correctArguments
      p.node.arguments[0].properties.some(
        p => p.type === "ObjectMethod" && p.key.name === "render"
      ) // hasRender
    );
  }
  ...
  addVariableDeclaration() {
    ...
    const variableDeclaration = this.types.variableDeclaration("var", [
      variableDeclarator
    ]);
  ...
  importDeclaration(name, from, isDefault = true) {
    ...
    const importSpecifier = isDefault
      ? this.types.importDefaultSpecifier(identifier)
      : this.types.importSpecifier(identifier, identifier);
    return this.types.importDeclaration(
      [importSpecifier],
      this.types.stringLiteral(from)
    );
  }
  ...
  getSuper(propsName = "props") {
    const callExpression = this.types.callExpression(this.types.super(), [
      this.types.identifier(propsName)
    ]);
  ...
  addPrepareComponentToConstructor(constructor, componentName, initialObject) {
    ...
    const callExpression = this.types.callExpression(memberExpression, [
      this.types.stringLiteral(componentName),
  ...
  prepareConstructor(componentName, classMethods, classProperties) {
    ...
    this.addPrepareComponentToConstructor(
      constructor,
      componentName,
      initialObject
    );
  ...
  classMethods(initialMethods) {
    ...
    for (let method of initialMethods) {
      if (
        this.isReactMethod(method.key.name) &&
        !this.isViewModelMethod(method.key.name)
      ) {
